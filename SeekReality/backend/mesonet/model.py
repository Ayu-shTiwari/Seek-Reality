...
def make_prediction(
    ...
    return_probs=False,
    return_report=False,
    invert_labels=False,  # New parameter to test label inversion
):
    """
    ...
    # Debug: Print class indices to verify mapping
    print("Class Indices:", data.class_indices)

    # Create label map based on directory structure
    label_map = {value: key.title() for key, value in data.class_indices.items()}
    if invert_labels:
        # Invert the mapping to test if labels were swapped during training
        label_map = {0: "Deepfake" if label_map[1] == "Real" else "Real",
                     1: "Real" if label_map[0] == "Deepfake" else "Deepfake"}
